...
class MainMenu:
    ...
    def run_window(self):
        ...
        button_game = Button(texture=self.reg.textures['ui']['button']['button_3'],
        ...
        button_designer = Button(texture=self.reg.textures['ui']['button']['button_3'],
                                 texture_motion=self.reg.textures['ui']['button']['button_3_motion'],
                                 texture_press=self.reg.textures['ui']['button']['button_3_pressed'],
                                 text='Конструктор карт',
                                 x=((self.setting_game.resolution[0] // 2) -
                                    (self.reg.textures['ui']['button']['button_3'].get_width() // 2)),
        ...
        button_settings = Button(texture=self.reg.textures['ui']['button']['button_3'],
                                 texture_motion=self.reg.textures['ui']['button']['button_3_motion'],
                                 texture_press=self.reg.textures['ui']['button']['button_3_pressed'],
                                 text='Настройки',
                                 x=((self.setting_game.resolution[0] // 2) -
                                    (self.reg.textures['ui']['button']['button_3'].get_width() // 2)),
                                 y=(self.setting_game.resolution[1] * 0.55),
                                 width=self.reg.textures['ui']['button']['button_3'].get_width(),
                                 height=self.reg.textures['ui']['button']['button_3'].get_height(),
                                 font=self.reg.fonts['Konstanting'],
                                 font_size=round(40 * self.setting_game.size_k[0]),
        ...
        button_exit = Button(texture=self.reg.textures['ui']['button']['button_3'],
                             texture_motion=self.reg.textures['ui']['button']['button_3_motion'],
                             texture_press=self.reg.textures['ui']['button']['button_3_pressed'],
                             text='Выход из игры',
                             x=((self.setting_game.resolution[0] // 2) -
        ...
        while running:
            ...
            button_designer.update(buttons[0], mouse_pos)
            button_designer.draw(self.screen)

            button_game.update(buttons[0], mouse_pos)
            button_game.draw(self.screen)

            button_settings.update(buttons[0], mouse_pos)
            button_settings.draw(self.screen)

            button_exit.update(buttons[0], mouse_pos)
            button_exit.draw(self.screen)

            self.screen.blit(cursor_image, mouse_pos)

            if button_game.is_up:
                levels = dict()
                ...
                if value is None:
                    ...
                    self.next_window = f'game_{value["name"]}'
                    running = False

            if button_designer.is_up:
                running = False
                self.next_window = 'map_new_or_load_menu'

            if button_settings.is_up:
                running = False
                self.next_window = 'settings_menu'

            if button_exit.is_up:
                running = False
                self.next_window = 'exit'
